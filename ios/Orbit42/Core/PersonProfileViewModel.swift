...
/// 차단 토글(`POST /api/v1/users/{username}/block`).
@MainActor
@Observable
final class PersonProfileViewModel {
    ...
    private(set) var isTogglingBlock = false
    ...
    /// 차단 / 차단 해제 토글. 성공 시 프로필을 다시 불러온다
    /// (차단하면 서버가 팔로우도 함께 정리하므로 낙관적 갱신 대신 재조회).
    func toggleBlock() async {
        guard let current = data, !isTogglingBlock else { return }
        ...
        do {
            let _: BlockResponse = try await api.post(
                ...
                body: BlockRequest(block: !current.isBlocked)
            )
